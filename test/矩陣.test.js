...
describe('測試【constructor】', () => {
	...
	it('傳入初始的rows與columns，rows與columns應等於傳入的數值', () => {
		let m = new 矩陣(5, 10);
		expect(m.直行數量).toBe(10);
		expect(m.橫列數量).toBe(5);
	});

	it('傳入多個陣列，每個元素應該與陣列元素相同', () => {
	...
	it('傳入多個陣列，中間穿插非陣列的資料，非陣列的資料應被忽略', () => {
...
describe('測試【特徵值】】', () => {
	it('[2,2,3,4][5,6,7,8][9,19,11,1][34,4,3,0]，應為-262', () => {
		const m = new 矩陣([2, 2, 3, 4], [5, 6, 7, 8], [9, 19, 11, 1], [34, 4, 3, 0]);
		expect(m.特徵值).toBe(-262);
	});

	it('[0,2,3,4][5,0,7,8][9,19,0,1][34,4,3,0]，應為362', () => {
...
describe('測試【列秩】】', () => {
	it('[2,2,3,1][4,5,6,3][7,8,3,3][2,2,2,0]，列秩等於4', () => {
...
describe('測試【上三角矩陣】】', () => {
	it('[2,2,3,1][4,5,6,3][7,8,3,3][2,2,2,0]', () => {
		const m = new 矩陣([2, 2, 3, 1], [4, 5, 6, 3], [7, 8, 3, 3], [2, 2, 2, 0]);
		const r = m.上三角矩陣();
...
describe('測試【冪次】】', () => {
	...
	it('[1,2][3,4]的4次應為[199,290][435,634]', () => {
		let m1 = new 矩陣([1, 2], [3, 4]);
		let m2 = m1.冪次(4);
...
describe('測試【加】】', () => {
	it('[1,2][3,4]加[5,6][7,8]應等於[6,8][10,12]', () => {
		let m1 = new 矩陣([1, 2], [3, 4]);
...
describe('測試【合併】】', () => {
	it('[2,2][2,2]合併[3,3]應為[2,2,3][2,3,3]', () => {
...
describe('測試【每一個橫列】】', () => {
	it('3x3矩陣用1填滿，應傳[1,1,1]給回呼函式', () => {
		new 矩陣(3, 3).填滿(1).每一個橫列((x) => {
			expect(x).toEqual([1, 1, 1]);
		});
	});

	it('3x3矩陣，應將橫列編號傳給回呼函式', () => {
		let 目前的橫列編號 = 0;
...
describe('測試【清除】】', () => {
	it('建立2x2矩陣，清除內容之後橫列數量和直行數量應等於0', () => {
		let m = new 矩陣(2, 2).清除();
		expect(m.橫列數量).toBe(0);
		expect(m.直行數量).toBe(0);
	});
...
describe('測試【減】】', () => {
	it('[1,2][3,4]減[5,6][7,8]應等於[-4,-4][-4,-4]', () => {
		let m1 = new 矩陣([1, 2], [3, 4]);
		let m2 = new 矩陣([5, 6], [7, 8]);
		let m3 = m1.減(m2);
...
describe('測試【直行】】', () => {
	it('[1,2][3,4]，直行(0)應等於[1,3]', () => {
		let m = new 矩陣([1, 2], [3, 4]);
		expect(m.直行(0)).toEqual([1, 3]);
	});

	it('[1,2][3,4]，直行(1)應等於[2,4]', () => {
		let m = new 矩陣([1, 2], [3, 4]);
...
describe('測試【複製】】', () => {
	it('[1,2,3][4,5,6]，複製之後應為[1,2,3][4,5,6]', () => {
...
describe('測試【設定橫列】】', () => {
	...
	it('矩陣[1,2,3][4,5,6][7,8,9]，設定橫列2 = [11,12,13]', () => {
		const m = new 矩陣([1, 2, 3], [4, 5, 6], [7, 8, 9]);
		m.設定橫列(2, [11, 12, 13]);
		expect(m[0][0]).toBe(1);
		expect(m[0][1]).toBe(2);
...
describe('測試【設定直行】】', () => {
	...
	it('矩陣[1,2,3][4,5,6][7,8,9]，設定直行2 = [11,12,13]', () => {
		const m = new 矩陣([1, 2, 3], [4, 5, 6], [7, 8, 9]);
		m.設定直行(2, [11, 12, 13]);
		expect(m[0][0]).toBe(1);
		expect(m[0][1]).toBe(2);
...
describe('測試【轉為字串】】', () => {
	it('[1,2,3][4,5,6]，轉為字串[1,2,3][4,5,6]', () => {
		const m = new 矩陣([1, 2, 3], [4, 5, 6]);
		const s = m.轉為字串();
		expect(s).toBe('[\r\n' +
			'  [ 1  2  3 ]\r\n' +
...
describe('測試【轉置矩陣】】', () => {
	...
	it('[1,2,3][4,5,6]，轉置之後應為[1,4][2,5][3,6]', () => {
		const m = new 矩陣([1, 2, 3], [4, 5, 6]);
		const t = m.轉置矩陣();
	...
	it('轉置是從m × n矩陣的向量空間到所有n × m矩陣的向量空間的線性映射。', () => {
