...
export default class 向量 extends Array {
    ...
    內積(另一個向量) {
        ...
        return 另一個向量.reduce((累加器, 當前值, 索引) => 累加器 + 當前值 * this[索引], 0, // 初始化內積值為 0
        );
    }
    ...
    外積(另一個向量) {
        ...
        return new 向量(y1 * z2 - z1 * y2, z1 * x2 - x1 * z2, x1 * y2 - y1 * x2);
    }
    ...
    有相鄰的(a, b) {
        ...
        return 型別.是數字(a) && 型別.是數字(b) ? 型別.兩個數值相等(下一個項目, b) : 下一個項目 === b;
    }
