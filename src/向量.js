...
module.exports = class 向量 extends Array {
	 ...
	 *
	 * @constructor
	 * @param {Array|number} [初始值=0] - 初始值，能接受一個數字或數組：
	 *    - 如果是數字，將用該數字初始化向量的長度，並以 0 填充內容。
	 *    - 如果是數組，將使用該數組的元素進行初始化。
	 ...
	 * 尋找最後一個符合條件的項目。
	 *
	 * 此方法會遍歷當前數組中的所有項目，根據提供的條件函式判斷哪些項目符合條件。
	 ...
	 * 尋找第一個符合條件的項目。
	 *
	 * 此方法會從數組中逐一檢查項目，根據提供的目標值與條件函式，返回第一個符合條件的項目。
	 * 若指定排除項目，排除的索引將不進行判斷。
	 *
	 ...
	 * 驗證索引是否在有效範圍內。
	 *
	 * 此方法用於檢查傳入的索引是否在數組的有效範圍內（`0 <= 索引 < this.length`）。
	 * 若索引無效（超出範圍），則拋出一個錯誤。
	 *
	 * @param {number} index - 要檢查的索引值。
	 *    - 必須是大於等於 0 並且小於此數組長度的有效索引。
	 *
	 * @throws {Error} - 當索引小於 0 或大於等於數組長度時，拋出錯誤。
