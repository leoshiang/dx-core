...
export default class 矩陣 extends Array {
    ...
    #以陣列做初始化(陣列集合) {
        ...
        陣列集合.filter(型別.是陣列).forEach(array => {
    ...
    #加減運算(目標, 加減數值, 加減矩陣) {
        const 加減運算 = 型別.是數字(目標) ? 加減數值 : (目標 instanceof 矩陣) ? 加減矩陣 : (() => {
    ...
    #從陣列集合計算維度(陣列集合) {
        return 陣列集合.reduce((結果, 陣列) => {
            if (型別.不是陣列(陣列)) {
                return 結果;
            }
            return {
                直行數量: Math.max(結果.直行數量, 陣列.length), 橫列數量: 結果.橫列數量 + 1,
            };
        }, {直行數量: 0, 橫列數量: 0});
    }
    ...
    #高斯約旦消去法計算反矩陣(矩陣) {
        ...
        for (let 對角線編號 = 矩陣.橫列數量 - 1; 對角線編號 > 0; 對角線編號--) {
            for (let r = 對角線編號; r > 0; r--) {
                ...
                const 消去橫列 = 矩陣.橫列(r - 1).加(矩陣.橫列(對角線編號).乘(-矩陣[r - 1][對角線編號]));
                矩陣.設定橫列(r - 1, 消去橫列);
            }
        }
    ...
    乘(目標) {
        ...
        回傳結果.forEach((_, row) => {
            回傳結果[row].forEach((_, column) => {
                回傳結果[row][column] = this[row].reduce((sum, value, c) => sum + value * 目標[c][column], 0);
            });
        });
    ...
    加(目標) {
        ...
        const 加上矩陣 = (橫列編號, 直行編號, 新矩陣) => 新矩陣[橫列編號][直行編號] = this[橫列編號][直行編號] +
            目標[橫列編號][直行編號];
    ...
    每一個元素(回呼函式) {
        this.forEach((橫列, 橫列編號) => 橫列.forEach((元素, 直行編號) => 回呼函式(元素, 橫列編號, 直行編號)));
        return this;
    }
    ...
    每一個直行(回呼函式) {
        this.#直行數量 &&
        Array.from({length: this.#直行數量}, (_, 直行編號) => 回呼函式(this.直行(直行編號), 直行編號));
        return this;
    }
    ...
    減(目標) {
        ...
        const 減去矩陣 = (橫列編號, 直行編號, 新矩陣) => 新矩陣[橫列編號][直行編號] = this[橫列編號][直行編號] -
            目標[橫列編號][直行編號];
    ...
    設定維度(新的橫列數量, 新的直行數量) {
        ...
        this.push(...Array.from({length: this.#橫列數量}, () => Array(this.#直行數量).fill(this.#預設值)));
        return this;
    }
    ...
    轉為字串() {
        ...
        this.forEach((row) => {
            回傳結果 += '  [' +
                row.reduce((acc, curr) => acc + ' ' + 型別.修正精確度誤差((型別.是很小的值(curr) ? 0 : curr)) + ' ',
                    '');
            回傳結果 += ']\r\n';
        });
        回傳結果 += ']\r\n';
