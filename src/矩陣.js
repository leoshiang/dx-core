...
module.exports = class 矩陣 extends Array {
    ...
    #參數必須是矩陣型別(目標) {
        if (!(目標 instanceof 矩陣)) throw new 型別錯誤('目標 的型別必須是 矩陣。')
    }
    ...
    乘(目標) {
        let 回傳結果
        if (型別.是數值(目標)) return this.#乘以倍數(目標)
        this.#參數必須是矩陣型別(目標)
     ...
     */
    冪次(n) {
        if (n === 0) return 矩陣.單位矩陣(this.橫列數量)
        if (n === 1) return this.複製()
    ...
    等於(目標) {
        ...
        if (this.#橫列數量 !== 目標.橫列數量 || this.#直行數量 !== 目標.直行數量) return false
        for (let 橫列編號 = 0; 橫列編號 < this.#橫列數量; 橫列編號++) {
            for (let 直行編號 = 0; 直行編號 < this.#直行數量; 直行編號++) {
                if (this[橫列編號][直行編號] !== 目標[橫列編號][直行編號]) return false
            }
        }
        return true
    }
    ...
    複製範圍(起始橫列編號, 起始直行編號, 橫列數量, 直行數量) {
        ...
        if (橫列數量 < 0) throw new Error('橫列數量必須大於零')
        if (直行數量 < 0) throw new Error('直行數量必須大於零')
        if (起始橫列編號 + 橫列數量 > this.#橫列數量) throw new Error('橫列數量超出範圍')
        if (起始直行編號 + 直行數量 > this.#直行數量) throw new Error('直行數量超出範圍')
